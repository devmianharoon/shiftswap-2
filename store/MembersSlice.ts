...
export const fetchCompanyMembers = createAsyncThunk<MemberData, { companyId: string; page?: number }, { rejectValue: string }>(
    ...
    async ({ companyId, page }, { rejectWithValue }) => {
        try {
            ...
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BE_URL}/company/${companyId}/members/?page=${page}`);
